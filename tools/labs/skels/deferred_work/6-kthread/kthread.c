...
int my_thread_f(void *data)
{
	...
	/* set flag to make kernel thead termination */
	atomic_set(&flag_thread_terminated, 1);
	/* notify the unload process that we have exited */
...
{
	/* notify the kernel thread that its time to exit */
