...
static const struct inode_operations minfs_dir_inode_operations = {
	.lookup		= minfs_lookup,
	/* Use minfs_create as the create function. */
	.create		= minfs_create,
};
...
static struct inode *minfs_iget(struct super_block *s, unsigned long ino)
{
	...
	/* Read block with inodes. It's the second block on
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		/* Use minfs_dir_inode_operations for i_op
	...
	/* Fill inode and file operations for regular files
	...
	/* uncomment after the minfs_inode is initialized */
	mii->data_block = mi->data_block;

	/* Free resources. */
	/* uncomment after the buffer_head is initialized */
	brelse(bh);
		...
		struct buffer_head **bhp)
{
	...
	*bhp = bh;
	...
	for (i = 0; i < MINFS_NUM_ENTRIES; i++) {
		...
		if (de->ino != 0 && strcmp(name, de->name) == 0) {
			final_de = de;
			break;
		}
	}
...
static int minfs_add_link(struct dentry *dentry, struct inode *inode)
{
	...
	/* get directory inode, containing inode & sb */
	dir = dentry->d_parent->d_inode;
	mii = container_of(dir, struct minfs_inode_info, vfs_inode);
	sb = dir->i_sb;

	/* Read dir data block (use sb_bread). */ 
	bh = sb_bread(sb, mii->data_block);
	if (bh == NULL) {
		printk(LOG_LEVEL "could not read block\n");
	}

	/* Find first free dentry (de->ino == 0). */	
	for (i = 0; i < MINFS_NUM_ENTRIES; i++) {
		de = ((struct minfs_dir_entry *) bh->b_data) + i;
		if (de->ino == 0) {
			break;
	...
	/* Place new entry in the available slot. Mark buffer_head
	 * as dirty. */
	de->ino = inode->i_ino;
...
static int minfs_fill_super(struct super_block *s, void *data, int silent)
{
	...
	/* use myfs_get_inode instead of minfs_iget */
...
static struct file_system_type minfs_fs_type = {
	.owner		= THIS_MODULE,
	.name		= "minfs",
	/* add mount, kill_sb and fs_flags */
	.mount		= minfs_mount,
