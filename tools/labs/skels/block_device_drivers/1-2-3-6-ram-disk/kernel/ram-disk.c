...
static void my_xfer_request(struct my_block_dev *dev, struct request *req)
{
	...
	rq_for_each_segment(bvec, req, iter) {
		...
		size_t len = bvec.bv_offset;
		int dir = bio_data_dir(iter.bio);

		/* copy bio data to device buffer */
		my_block_transfer(dev, sector, len, buffer, dir);
...
static void my_block_request(struct request_queue *q)
{
	...
	while (1) {
		...
		pr_info("request received start sector : %lu, total size : %u,\
			current bio data size : %d , direction : %c \n",
			(long unsigned)blk_rq_pos(rq),
